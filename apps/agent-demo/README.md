...
- Two live jobs verified the payment-and-analysis path. Circle later reported the recorded Gateway transfer as `completed`, and ArcScan reports its batch transaction as successful. Sustained traffic and a real 24-hour cleanup cycle have not been observed. The paid browser flow passed; its final isolation assertion read the new session before initialization. An explicit wait fixes that timing error, and a separate read-only production isolation test passed.
