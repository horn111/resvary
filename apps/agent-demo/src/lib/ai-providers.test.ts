      ...
      choices: [
        {
          index: 0,
          finish_reason: 'stop',
          message: { role: 'assistant', content: 'Known result' },
        },
      ],
...
describe('bounded AI provider transport', () => {
  it('resolves separate roles and accepts existing key aliases without persisting credentials', () => {
    const agent = resolveAiConfig('agent', {
      ...env,
      AGENT_DEMO_AGENT_MODEL: 'openai/gpt-4.1-mini',
    });
  ...
  it('rejects unsupported models, object property names and altered tariff snapshots', () => {
    for (const model of ['not-reviewed', '__proto__', 'constructor']) {
      expect(() => resolveAiConfig('agent', { ...env, AGENT_DEMO_AGENT_MODEL: model })).toThrow(
        ProviderRejected,
      );
    }
  ...
  it('maps token usage and refuses missing, fractional or negative counts', () => {
    expect(normalizeAiUsage({ prompt_tokens: 3, completion_tokens: 4 })).toEqual({
      input_tokens: '3',
      output_tokens: '4',
    });
    for (const value of [undefined, -1, 1.5, '4', NaN]) {
      expect(() => normalizeAiUsage({ prompt_tokens: 3, completion_tokens: value })).toThrow(
        ProviderOutcomeUnknown,
      );
    }
  });
  ...
  it('rejects oversized input before sending any request', async () => {
    const transport = vi.fn(async () => completion());
    await expect(
      analyzeWithProvider('x'.repeat(20_000), { ...options, fetch: transport }),
    ).rejects.toThrow();
  ...
  it('does not retry server failures or transport timeouts', async () => {
    for (const response of [
      ...
      await expect(
        analyzeWithProvider('Fixture.', { ...options, fetch: transport }),
      ).rejects.toThrow();
  ...
  it('propagates the run deadline into the provider request without retrying', async () => {
    ...
    await expect(
      analyzeWithProvider('Fixture.', { ...options, signal: controller.signal, fetch: transport }),
    ).rejects.toThrow();
    expect(transport).toHaveBeenCalledTimes(1);
  });
  it('distinguishes an HTTP rejection from an incomplete generated answer', async () => {
    await expect(
      analyzeWithProvider('Fixture.', {
        ...options,
    ...
    ).rejects.toBeInstanceOf(ProviderOutcomeUnknown);
  });
  it('uses native tool calls through the installed Agents SDK', async () => {
              ...
              tool_calls: [
                {
                  id: 'call_fixture',
                  type: 'function',
                  function: { name: 'top_up', arguments: '{}' },
                },
              ],
            },
          },
    ...
    const agent = new Agent({
      ...
      tools: [
        tool({ name: 'top_up', description: 'Fixture only.', parameters: z.object({}), execute }),
      ],
      toolUseBehavior: 'stop_on_first_tool',
      modelSettings: { maxTokens: 512, parallelToolCalls: false, retry: { maxRetries: 0 } },
    });
    const runner = new Runner({
      tracingDisabled: true,
      modelProvider: createAgentModelProvider(config, { env, fetch: transport }),
    });
