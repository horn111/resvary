import { CreditNotFoundError, signCreditOutboxEvent } from '@settlary/sdk/credits';
import { ArcCreditFunding } from '@settlary/sdk/funding/arc';
import { createReceipt } from '@settlary/sdk/receipts';
  ...
  return {
    ...
    latestSignature: latestEvent
      ? signCreditOutboxEvent(
          latestEvent,
          process.env.SETTLARY_WEBHOOK_SECRET ?? 'settlary-demo-secret',
        ).header
      : null,
    liveProviderConfigured: Boolean(
      process.env.SETTLARY_AI_API_KEY && process.env.SETTLARY_AI_MODEL,
    ),
    persistence: process.env.SETTLARY_CREDITS_DB_PATH ?? '.settlary/demo.sqlite',
...
async function simulateProvider(idempotencyKey: string) {
  return {
    value: { text: 'A simulated answer generated by the Settlary demo.' },
...
async function callLiveProvider() {
  const apiKey = process.env.SETTLARY_AI_API_KEY;
  const model = process.env.SETTLARY_AI_MODEL;
  const baseUrl = (process.env.SETTLARY_AI_BASE_URL ?? 'https://api.openai.com/v1').replace(
    /\/$/,
    '',
  );
