...
If a defect is found after the first package is public, do not reuse that version. Fix it in the next patch release. If a transient registry or network failure causes a partial publication, rerun `publish` only while the release commit is still the workflow's `main` SHA. If the workflow itself must be fixed first, GitHub's built-in `GITHUB_TOKEN` cannot create a tag on the older release commit after workflow files change. Complete the matching immutable artifacts and tag through an explicitly authenticated recovery, then run `reconcile` with the original version, `release_sha`, and published console digest to verify the result without mutating it.
