...
For the `0.8.0` evidence run, copy `.env.example` to `.env.local`, fund a disposable Arc
...
`docs/evidence/0.8.0/arc-testnet-proof.json`; the private key is never written to evidence or logs.
