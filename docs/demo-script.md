...
2. Click **Run simulated AI**. Settlary reserves the maximum token cost, executes deterministic simulated usage, charges the actual amount, and releases the remainder.
...
4. Inspect the immutable ledger and transactional outbox. The latest event includes an `x-settlary-signature`-compatible header.
...
Set `SETTLARY_AI_API_KEY` and `SETTLARY_AI_MODEL`; optionally set `SETTLARY_AI_BASE_URL`. The UI then exposes **Run live provider**. The route expects an OpenAI-compatible chat completions response with prompt and completion token usage.
