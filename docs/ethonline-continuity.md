...
The paid production browser flow passed its payment, analysis, replay, and leftover-credit checks. Its final isolation assertion read status before the new session finished initialization. The test now waits for initialization, and a separate read-only production run verified isolation without creating another paid job. Three private screenshots capture the paid flow.
...
Gateway transfer `79c04414-c4ec-4ccd-95b8-24df4e8b6e3b` independently reported the expected 20,000 USDC base units, payer, seller, and Arc Testnet network. During the recorded run its status was `received` with no transaction hash because Gateway accepts nanopayments before [batch settlement onchain](https://developers.circle.com/gateway/nanopayments/concepts/batched-settlement). Circle later reported the transfer as `completed`, and [ArcScan reports the batch transaction as successful](https://testnet.arcscan.app/tx/0xa05a7f9351aa59f20e2872cb9c5535227c242cbc58a4fd4ed03b40076781c0b1). The proof preserves both observations and keeps Resvary's facilitator-level funding status separate from onchain finality. Real 24-hour cleanup and sustained-load behavior remain unverified.
