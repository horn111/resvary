...
describe('starter templates', () => {
  ...
  it('creates valid package manifests for both starter types', () => {
    ...
    expect(ai.dependencies['@resvary/sdk']).toBe('0.8.0');
    expect(ai.dependencies['@resvary/sqlite']).toBe('0.8.0');
  ...
  it('makes AI credits the functional embedded SDK starter', () => {
    ...
    expect(JSON.parse(packageTemplate(postgres)).dependencies['@resvary/postgres']).toBe('0.8.0');
    expect(JSON.parse(packageTemplate(postgres)).dependencies['@resvary/worker']).toBe('0.8.0');
